...
export async function generateMetadata({
  ...
  // Only index pages with real generated content
  let hasContent = false
  try {
    const fs = require('fs')
    const path = require('path')
    hasContent = fs.existsSync(path.join(process.cwd(), 'content', 'blog', 'generated', `${slug}.html`))
  } catch {}
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <article className="mx-auto max-w-3xl px-4 py-16 lg:py-24">
        ...
        {/* Content — loaded from generated HTML files */}
        <div className="prose prose-invert prose-violet max-w-none">
          {(() => {
            try {
              const fs = require('fs')
              const path = require('path')
              const filePath = path.join(process.cwd(), 'content', 'blog', 'generated', `${post.slug}.html`)
              if (fs.existsSync(filePath)) {
                const html = fs.readFileSync(filePath, 'utf-8')
                return <div dangerouslySetInnerHTML={{ __html: html }} />
              }
            } catch {}
            return <div className="rounded-2xl border border-violet-500/10 bg-violet-500/[0.02] p-8 text-center">
              <p className="text-muted-foreground">This article is being generated by our AI pipeline. Check back soon!</p>
            </div>
          })()}
        </div>
